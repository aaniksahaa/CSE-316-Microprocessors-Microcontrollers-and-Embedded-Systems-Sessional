...
def calculate_hsv_avg(image, bbox):
    ...
    h_max = min(np.max(hsv_roi[:,:,0]) + COLOR_DEVIATION, 255)
    s_max = min(np.max(hsv_roi[:,:,1]) + COLOR_DEVIATION, 255)
    v_max = min(np.max(hsv_roi[:,:,2]) + COLOR_DEVIATION, 255)
    h_min = max(np.min(hsv_roi[:,:,0]) - COLOR_DEVIATION,0)
    s_min = max(np.min(hsv_roi[:,:,1]) - COLOR_DEVIATION,0)
    v_min = max(np.min(hsv_roi[:,:,2]) - COLOR_DEVIATION,0)
    
    return np.array([h_avg, s_avg, v_avg]), np.array([h_min, s_min, v_min]), np.array([h_max, s_max, v_max])
...
def draw_bounding_box_from_hsv(image, lower,upper):
    red_contours, red_centers = get_detected_object_centers(image, lower, upper, 10)
...
while True:
    ...
    if key == ord('q'):
        ...
        if bbox_start and bbox_end:
            ...
            image = orig_image
            avg, lower, upper = calculate_hsv_avg(image, bbox)
            ...
            cv2.resizeWindow('hehe', 1920, 1080)
